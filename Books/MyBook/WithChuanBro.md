...
场景:本系统批量处理数据发送至第三方系统

涉及流程:查询需要处理的ID=》for{封装需要处理的数据> 发送数据=>记录日志}

具体产生问题:查到的id关联数据在for里报错空指针异常，导致程序直接终止。下一次查询还是从空指针异常，阻塞后边的数据处理。解决方案:需要增加日志和补货异常。在for最容易出错的地方写trycatch在catch里continue
...
- 2022-09-15：发现代码未完成，但是晚上就要上线了，随随便便就测试，然后硬着头皮上线。这明显是不对的。

# 9.业务

## 9.1签收问题

![image-20220927000530053](mdimg/image-20220927000530053.png)
...
目前终端业务分为（1）手机终端和（2）家业终端，两种业务类型下单逻辑截然不同。应客户要求上线分货，但是只维护了手机终端业务，并没有考虑家业终端，导致家业无限采购。
